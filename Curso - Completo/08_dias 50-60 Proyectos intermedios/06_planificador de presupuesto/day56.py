...
def set_saving_goal():
    goal = float(input('Enter your monthly saving goal: '))
    print(f'Your saving goal is set to ${goal:.2f}')
    return goal
...
def check_saving_goal(remaining, goal):
    if remaining >= goal:
        print(f'Congratulations! You have met your saving goal with ${remaining - goal:.2f} extra.')
    else:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            check_saving_goal(remaining, goal)
        elif choice == '5':
            plot_expenses()
        elif choice == '6':
            print('Goodbye!')
